    ...
    return Scaffold(
      ...
      floatingActionButton: Column(
        ...
        children: [
          ...
          HoverExpandFab(
            heroTag: 'novaLicitacao',
            onPressed: () => context.go('/licitacao/new'),
            icon: Icons.add,
            tooltip: 'Nova Licitação',
          ),
        ],
      ),
      body: _LicitacaoList(status: _statusFilter, search: _searchCtrl.text),
    );
  }
}
...
class _LicitacaoList extends ConsumerWidget {
  final String status;
  final String search;
  const _LicitacaoList({required this.status, this.search = ''});
    ...
    return stream.when(
      ...
      data: (licitacoes) {
        final filtered = search.isEmpty
            ? licitacoes
            : licitacoes.where((l) {
                final edital = editaisAsync.value?[l.editalId];
                final searchable = [
                  l.codigoEdital,
